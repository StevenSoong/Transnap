...
1. 全局快捷键提供当前鼠标位置。
2. `SelectionReader` 读取选中文本并确定锚点；短选区会尝试附带有限长度的前后文。
...
应用只向用户配置的基础地址发起 `POST /chat/completions` 请求。请求包含模型名称、翻译提示词和当前选中的原文。对于不含空格且不超过 64 个字符的短选区，请求还可能包含同一文本控件前后各最多 240 个字符，用于消除词义歧义。上下文不会显示、写入历史或在后续请求中复用；既有翻译历史也不会作为上下文发送。
